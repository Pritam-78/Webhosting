...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    h1 { font-size: 3rem; margin-bottom: 16px; background: linear-gradient(135deg, #6c63ff, #a855f7); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
...
async function publishSite() {
  ...
  try {
    ...
    if (!res.ok) {
      showError('publishError', data.error || 'Failed to publish site.');
      return;
    }
    ...
    showError('publishError', 'Network error. Please try again.');
  } finally {
    btn.disabled = false;
  ...
  grid.innerHTML = sites.map(site => `
    <div class="site-card" id="site-${site.id}">
      ...
      <div class="site-card-actions">
        <button class="btn btn-outline btn-sm" onclick="copyToClipboard('${host}/site/${site.slug}', this)">
          <i class="fa-solid fa-copy"></i> Copy Link
        </button>
        <a href="${host}/site/${site.slug}" target="_blank" class="btn btn-ghost btn-sm">
          <i class="fa-solid fa-arrow-up-right-from-square"></i> Visit
        </a>
        <button class="btn btn-sm" style="background:var(--danger-bg);color:var(--danger);border:1px solid rgba(239,68,68,0.3);" onclick="openDelete(${site.id})">
...
async function confirmDelete() {
  ...
  try {
    const res = await fetch(`/api/sites/${deleteTargetId}`, {
      method: 'DELETE',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
    ...
    const data = await res.json();
    if (!res.ok) {
      showError('deleteError', data.error || 'Failed to delete site.');
      return;
    }
    closeDelete();
